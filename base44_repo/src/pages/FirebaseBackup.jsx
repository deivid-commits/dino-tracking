    ...
    try {
      ...
      try {
        ...
        if (data.success) {
          setResult({ 
            type: 'success', 
            message: `✅ Conexión exitosa! Completado en ${data.diagnostics.duration_ms}ms` 
          });
        } else {
          setResult({ 
            type: 'error', 
        ...
        clearTimeout(timeoutId); // Clear timeout even on fetch error
        if (fetchError.name === 'AbortError') {
          throw new Error('Request timeout after 60 seconds - la función backend no responde');
        }
        throw fetchError;
      }
      ...
      if (error.message.includes('timeout') || error.message.includes('abort')) {
        errorMessage = '⏱️ Timeout: La función de Firebase no respondió a tiempo. Posibles causas:\n\n' +
          '1. Las credenciales de Firebase pueden ser incorrectas\n' +
          '2. El proyecto de Firebase no existe o no es accesible\n' +
          '3. Hay un problema de red entre Deno Deploy y Firebase\n\n' +
          'Por favor, verifica tus credenciales en Dashboard → Settings → Environment Variables';
      }
