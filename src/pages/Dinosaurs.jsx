...
export default function DinosaurusPage() {
  ...
  // Function to generate unique toy_id
  const generateToyId = async () => {
    ...
    while (!isUnique) {
      toyId = 'TOY_';
      for (let i = 0; i < 6; i++) {
        toyId += characters.charAt(Math.floor(Math.random() * characters.length));
      }
      
      // Check if this toy_id already exists
      const existing = await Dinosaur.filter({ toy_id: toyId });
  ...
  const handleQuickRegister = async (e) => {
    ...
    try {
      ...
      // Generate unique toy_id
      const toyId = await generateToyId();
  ...
  const filteredDinosaurs = dinosaurs.filter(dino => {
    const matchesSearch = searchTerm === "" || 
      ...
      (dino.toy_id && dino.toy_id.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Input
              placeholder={t('search_rfid_device_placeholder')} // This will now also search toy_id
              value={searchTerm}
